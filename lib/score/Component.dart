import 'package:abjad/duration.dart' as durationLib;
import 'package:abjad/system.dart' as systemLib;
import 'package:abjad/math.dart' as mathLib;
...
class Component extends AbjadObject{
  ...
  **/
  ...
  var _after_grace;
  var _dependent_expressions;
  var _grace;
  var _indicators;
  var _indicators_are_current;
  var _is_forbidden_to_update;
  var _lilypond_grob_name_manager;
  var _lilypond_setting_name_manager;
  var _offsets_are_current;
  var _offsets_in_seconds_are_current;
  var _parent;
  var _spanners;
  var _start_offset;
  var _start_offset_in_seconds;
  var _stop_offset;
  var _stop_offset_in_seconds;
  var _timespan;
  var _is_counttime_component = false;
  ...
  Component(){
    ...
    _grace = null;
    _indicators = [];
    _indicators_are_current = false;
    _is_forbidden_to_update = false;
    _offsets_are_current = false;
    _offsets_in_seconds_are_current = false;
    _lilypond_grob_name_manager = null;
    _parent = null;
    _lilypond_setting_name_manager = null;
    _spanners = set();
    _start_offset = null;
    _start_offset_in_seconds = null;
    _stop_offset = null;
    _stop_offset_in_seconds = null;
    _timespan = timespan.Timespan();
  }

  Component copy(){
    /** 
    * Copies component with indicators but without children of component
  ...
  operator *(n){
      ...
      **/
      var result = mutate(self).copy(n=n);
  ...
  _cache_named_children() {
    var name_dictionary = {}
    if (_named_children){
      _named_children.forEach(name, children){
        name_dictionary[name] = copy.copy(children);
      }
    }
    if (name && name != null){
      if (! name_dictionary.containskey(name)){
        name_dictionary[name] = [];
      }
      name_dictionary[name].push(this);
    }
